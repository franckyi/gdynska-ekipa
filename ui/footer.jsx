...
export default async function Footer({info}) {
    return (
        <footer className="pb-8 pt-12 w-4/6 mx-auto">
            ...
            <div className="mt-4">Gdyńska Ekipa Dawid Bierkus</div>
            <div className="mb-4">NIP: {info.meta.nip}</div>
            <div className="my-4">{info.meta.address}</div>
            <a className="block hover:text-amber-600" title="Email" href="mailto:dawid@gdynskaekipa.pl">dawid@gdynskaekipa.pl</a>
            <a className="block hover:text-amber-600" title="Email" href={`mailto:${info.meta.email}`}>{info.meta.email}</a>
            <a className="block mt-4 hover:text-amber-600" title="Phone number" href="tel:605-150-157">Dawid Bierkus 605_150_157</a>
            <a className="block hover:text-amber-600" title="Phone number" href={`tel:${info.meta.phone}`}>Anna Bierkus {info.meta.phone}</a>
            <a className="block mt-4 hover:text-amber-600" title="Facebook page" href={info.meta.facebook_url} target="_blak">Facebook</a>
